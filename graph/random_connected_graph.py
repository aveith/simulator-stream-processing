...
class Graph(object):
    ...
    def dotprint(self, fname):
        with open(fname, mode='w') as f:
            f.write('digraph graphname {')
            for source, target in self.edges:
                f.write('  %d -> %d; '% (self.nodes.index(source), self.nodes.index(target)))
            f.write('}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-k', '--dot',
                        help='filename to save the graph to in DOT format')

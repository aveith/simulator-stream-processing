...
class experiment_parameter:
    def __init__(self, ):
        ...
        self.reconfig_strategies = [1, 3, 4]
        self.config_strategies = [1, 2, 3, 4]
        self.reconfig_constants = [.005, .1, 2]
        self.reconfig_alpha = [.001, .01, .1, .9]  # no
        self.reconfig_gamma = [.005, .1, 2]
        self.reconfig_lambda = [.005, .1, 2]
        self.reconfig_epsilon = [.001, .01, .1, .9, 1]  # no
        self.reconfig_price_model = [0, 1]
        self.reconfig_scenario = [[1, 0, 0, 0], [.25, .25, .25, .25], [.4, .2, .2, .2]]
        self.network = 'n3.xml'
        self.app = 'dot/a1.dot'
...
class experiment_configuration:
    def __init__(self, id=0, directory='',
                 ...
                 app='a1', reconfig_price_model=0, latency_weight=1, migration_weight=0, costs_weight=0,
                 wan_traffic_weight=0, seeds=[], configuration=0, tracing=False, config_strategy=-1, network="n3.xml",
                 file_path_stat="", file_operator_stat="", source_location=0, sink_location=0):
        if migration_weight == .2:
            s = 0
            s = s + 1
        self.id = id
...
def build_tracing_scenarios(dir='', dir_trace_datasets='', seed=3454326, configurations=1,
                            parameters=experiment_parameter(), source_location=0, sink_location=0):
    _scenarios = []
    id = 0
    ...
    for c in range(0, int(configurations)):
        ...
        for st in range(len(parameters.config_strategies)):
            for a in range(len(parameters.app)):
                ...
                exp.network = parameters.network
                exp.file_path_stat = parameters.file_path_stat
                exp.file_operator_stat = parameters.file_operator_stat
                exp.source_location = source_location
                exp.sink_location = sink_location
                            ...
                            for ep in range(len(epsilon)):
                                for pm in range(len(parameters.reconfig_price_model)):
                                    for sc in range(len(parameters.reconfig_scenario)):
                                        ...
                                        exp.source_location = traced_scenarios[tr].source_location
                                        exp.sink_location = traced_scenarios[tr].sink_location
                                        ...
                                        logging.warning(
                                            ...
                                            parameters.reconfig_scenario[sc][3], constants[ct], alpha[al], gamma[gm],
...
def build_ini_file(_scenario=experiment_configuration(), _dir=''):
    ...
    _replace = [_name,
                ...
                "latency_weight = " + str(_scenario.latency_weight),
                "migration_weight = " + str(_scenario.migration_weight),
                "costs_weight = " + str(_scenario.costs_weight),
                "wan_traffic_weight = " + str(_scenario.wan_traffic_weight),
                "pricing_type = " + str(_scenario.reconfig_price_model),
                ...
                "source_locations = " + str(_scenario.source_location),
                "sink_locations = " + str(_scenario.sink_location)]
    _base = 3454326
    for s in range(56):
...
def execute_simulation(_scenario=experiment_configuration()):
    ...
    if _scenario.tracing:
        logging.warning('Finished tracing scenario\t%d time(seconds)\t%f strategy\t%d app\t%s ', _scenario.id,
                        difference, _scenario.config_strategy, _scenario.app)
    else:
        logging.warning('Finished simulated scenario\t%d time(seconds)\t%f strategy\t%d reconfg\t%d app\t%s ',
                        _scenario.id, difference, _scenario.config_strategy, _scenario.reconfig_strategy, _scenario.app)
...
def main(dir='', dir_trace_datasets='', network='', app='', reconfig_strategy=[], latency_weight=-1, downtime_weight=-1,
         monetary_cost_weight=-1, wan_traffic_weight=-1, pricing_type=0, config_strategy=[], reconfig=False,
         config=False,
    ...
    parameters.reconfig_strategies = []
    parameters.reconfig_strategies = reconfig_strategy
    parameters.config_strategies=[]
    parameters.config_strategies = config_strategy

    parameters.reconfig_price_model = []
    parameters.reconfig_price_model = [pricing_type]

    parameters.reconfig_scenario = []
    parameters.reconfig_scenario = [[latency_weight, downtime_weight, monetary_cost_weight, wan_traffic_weight]]
    ...
    tracing_scenarios = build_scenarios(dir=dir,
                                        ...
                                        configurations=1,
                                        tracing=True, parameters=parameters, source_location=source_location,
    ...
    if config:
        if tracing_scenarios.__len__() > 0:
            # execute_simulation(tracing_scenarios[0])
